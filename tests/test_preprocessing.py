...
class TestNormalize:
    def test_minmax_normalization(self):
        ...
        expected = tv_tensors.Image(torch.tensor([0.0, 1/3, 2/3, 1.0]).view(1, 1, 2, 2))
    ...
    def test_zscore_normalization(self):
        ...
        mean = 2.5
        std = 1.2909944
        expected = tv_tensors.Image(torch.tensor([(1-mean)/std, (2-mean)/std, 
                                                 (3-mean)/std, (4-mean)/std]).view(1, 1, 2, 2))
        assert torch.allclose(result, expected)
